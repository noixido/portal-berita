...
@section('content')
    <div class="p-6">
        <form action="{{ route('proses-edit-berita', $beritas->id) }}" method="POST" class="flex flex-wrap w-full gap-2"
            ...
            <div class="mb-5 w-2/5">
                <label for="judul" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Judul
                    Berita</label>
                <input type="text" id="judul" name="judul" value="{{ old('judul', $beritas->judul) }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Ini Judul Berita" required />
            </div>
            <div class="mb-5 w-2/5">
                <label for="slug" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Slug</label>
                <input type="text" id="slug" name="slug" value="{{ old('slug', $beritas->slug) }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    readonly />
            </div>
            <div class="mb-5 w-5/6">
                ...
                <textarea name="deskripsi" rows="4"
                    ...
                    placeholder="Tuliskan deskripsi berita..." required>{{ $beritas->deskripsi }}</textarea>
            </div>
            <div class="mb-5 w-2/5">
                <label for="thumbnail"
                ...
                <input name="thumbnail"
                    ...
                    id="thumbnail" type="file" onchange="previewImage()">
                @if ($beritas->thumbnail)
                    <img src="{{ asset('storage/' . $beritas->thumbnail) }}" class="rounded mt-2 mx-auto w-60"
                        id="preview">
                @else
            ...
            <div class="mb-5 w-2/5">
                ...
                <select name="kategori"
                    ...
                    @foreach ($kategoris as $item)
                        @if (old('kategori', $beritas->kategori_id) == $item->id)
            ...
            <div class="mb-5 w-2/5">
                ...
                <select name="status_berita"
                    ...
                    <option value="headline"
                        {{ 'headline' === old('status_berita', $beritas->status_berita) ? 'selected' : '' }}>Headline
                    </option>
                    <option value="regular"
                        {{ 'regular' === old('status_berita', $beritas->status_berita) ? 'selected' : '' }}>Regular
                    </option>
                </select>
            </div>
            <div class="mb-5 w-2/5">
                ...
                <select name="status_publish"
                    ...
                    <option value="publish"
                        {{ 'publish' === old('status_publish', $beritas->status_publish) ? 'selected' : '' }}>Publish
                    </option>
                    <option value="draft"
                        {{ 'draft' === old('status_draft', $beritas->status_publish) ? 'selected' : '' }}>Draft</option>
                </select>
            </div>
            <div class="mb-5 w-2/5">
                <button type="submit"
